...
$to = "kiku.mitsu.yoshi.taka@gmail.com";
...
$require = array('お名前', 'メールアドレス', 'ご用件', 'お問い合わせ内容');
...
?>
	...
	<head>
		...
		<title>確認画面</title>
		<link rel="stylesheet" href="inc/formstyle.css">
	</head>
	...
	<body id="Home" class="bg-light bg-img">
		...
		<div class="container bg-white py-5 my-5">
			...
			<!-- お問い合わせ-->
			<div id="about" class="">
					<h2 class="text-center pt-3 m-title ">確認画面</h2>
			...
			<div id="formWrap">
				...
				<?php } else { ?>
					<p class="text-center">以下の内容で間違いがなければ、「送信する」ボタンを押してください。</p>
					<form action="<?php echo h($_SERVER['SCRIPT_NAME']); ?>" method="POST">
						<table class="formTable">
							<?php echo confirmOutput($_POST); //入力内容を表示
							?>
						</table>
						<p class="text-center m-5"><input type="hidden" name="mail_set" value="confirm_submit">
...
?>
	...
	<body id="Home" class="bg-light bg-img">
		...
		<div class="container bg-white py-5 my-5">
			...
			<!-- お問い合わせ-->
			<div id="about" class="">
					<h2 class="text-center pt-3 title ">メッセージが送信されました</h2>
			</div>
			<div class="text-center">
				<?php if ($empty_flag == 1) { ?>
					<h4>入力にエラーがあります。下記をご確認の上「戻る」ボタンにて修正をお願い致します。</h4>
			...
			function refererCheck($Referer_check, $Referer_check_domain)
			{
				if ($Referer_check == 1 && !empty($Referer_check_domain)) {
					if (strpos($_SERVER['HTTP_REFERER'], $Referer_check_domain) === false) {
						return exit('<p class="text-center">リファラチェックエラー。フォームページのドメインとこのファイルのドメインが一致しません</p>');
					}
				}
			}
			function copyright()
